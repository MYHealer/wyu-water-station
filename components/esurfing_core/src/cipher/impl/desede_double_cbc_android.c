/**
 * Double 3DES-CBC — 从 CVersion desede_double_cbc_android.c 移植
 * Algo Id: 9ABF4D29-34DB-4CE9-BB8C-7E371D637758
 *
 * 两层 3DES-CBC: pass1 key[24..47], pass2 key[0..23],
 * 标准 DES core, CBC 模式 8 字节 IV, 零填充到 8 字节.
 */
                                          ...
                                          const size_t data_len, size_t* output_len)
{
    ...
    uint8_t *out = calloc(1, total ? (size_t)total : DESEDE_DOUBLE_BLOCK_SIZE);
    if (!out) return NULL;
...
cipher_interface_t* create_desede_double_cbc_android_cipher(const uint8_t* key, const uint8_t* iv)
{
    ...
    desede_double_cbc_android_data_t* d = malloc(sizeof(desede_double_cbc_android_data_t));
