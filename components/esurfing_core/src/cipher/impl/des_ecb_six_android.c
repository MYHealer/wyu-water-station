/**
 * Six-layer DES-ECB — 从 CVersion des_ecb_six_android.c 移植
 * Algo Id: AD8BB5B0-0E72-4198-A362-96D52C1B7ED1
 *
 * 每个 8 字节块六层 DES: E(K1) D(K2) E(K3) E(K4) D(K5) E(K6)
 * K1..K6 = key[0..7], key[8..15], ..., key[40..47], ECB 模式.
 */
                                    ...
                                    const size_t data_len, size_t* output_len)
{
    ...
    uint8_t *out = calloc(1, total ? (size_t)total : DES_SIX_BLOCK_SIZE);
    if (!out) return NULL;
...
cipher_interface_t* create_des_ecb_six_android_cipher(const uint8_t* key)
{
    ...
    des_ecb_six_android_data_t* d = malloc(sizeof(des_ecb_six_android_data_t));
