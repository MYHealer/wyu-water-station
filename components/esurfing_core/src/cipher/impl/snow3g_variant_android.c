/**
 * SNOW3G-variant — 从 CVersion snow3g_variant_android.c 移植
 * Algo Id: 07E824B2-9E5C-4D1B-BBB0-5E07C251E4AA
 *
 * SNOW3G 流密码: 16 s-cells + FSM (两个 S-box),
 * 16 轮 key/iv 混合 + 32 轮预热, 然后 keystream XOR.
 * 零填充到 4 字节倍数 (流模式).
 */
                                   ...
                                   const size_t data_len, size_t* output_len)
{
    ...
    uint8_t* buf = calloc(1, total ? (size_t)total : 1);
    if(!buf) return NULL;
...
static uint8_t* snow3g_decrypt_raw(const uint8_t* key, const uint8_t* iv, const uint8_t* data,
                                   const size_t data_len, size_t* output_len)
{
    uint8_t* buf = malloc(data_len ? data_len : 1);
    if(!buf) return NULL;
    memcpy(buf, data, data_len);
    uint32_t c[SNOW3G_CELLS];
    snow3g_keyschedule(c, key, iv);
...
cipher_interface_t* create_snow3g_variant_android_cipher(const uint8_t* key, const uint8_t* iv)
{
    ...
    snow3g_variant_android_data_t* d = malloc(sizeof(snow3g_variant_android_data_t));
