/**
 * Double-layer AES-128-CBC — 从 CVersion aes_double_cbc_android.c 移植
 * Algo Id: BB2EA626-590B-4C42-82BE-E052FCBBB88E
 *
 * 两层 AES-128-CBC (32 字节 key = 两个 16 字节层),
 * 自定义 key expansion (转置 key + 10 轮 S-box/Rcon 混合 -> 176 字节 schedule),
 * 标准 AES 轮结构, 行优先 state, IV 前缀模式, 零填充.
 */
                                         ...
                                         const uint8_t iv[16], int* out_len)
{
    int total = (len + 15) & ~15;
    if (total == 0) total = 16;
    uint8_t *buf = calloc(1, (size_t)total + 32);
    if (!buf) return NULL;
...
cipher_interface_t* create_aes_double_cbc_android_cipher(const uint8_t* key, const uint8_t* iv)
{
    ...
    aes_double_cbc_android_data_t* d = malloc(sizeof(aes_double_cbc_android_data_t));
